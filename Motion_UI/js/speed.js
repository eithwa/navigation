...
if (typeof(Storage) !== "undefined") {
    ...
    if (localStorage.getItem("MAX_SPEED") != null) {
        ...
        if(localStorage.MAX_SPEED<=10){
            document.getElementById("SpeedInput").step = 0.1;
            document.getElementById("SpeedSlider").step = 0.1;
        }else{
            document.getElementById("SpeedInput").step = 1;
...
function set_speed_topic(value){
   ...
   cmdVel1.name = localStorage.SPEED_TOPIC
}
...
function set_speed(value){
    if(value>=0 && value<=100){
        localStorage.SPEED = parseFloat(value);
        //console.log("localStorage.SPEED", localStorage.SPEED);
    }
}
