...
* 若更換平台需自行更換motion控制
 ...
 - fira平台建議先調整（乘以30倍）再輸出到馬達,若未調整速度過慢將會無法移動  
...
```
* 開啟navigation的launch前先將launch內雷射的node關閉
...
```
  ...
  <!--node pkg="hokuyo_node" type="hokuyo_node" name="hokuyo_node">
    <param name="port" value="/dev/sensors/hokuyo"/>
  </node-->
```
* 如果要使用amcl_navigation_by_odom.launch
  - 開啟fira6/fira6_description/urdf/fira6.gazebo.xacro 修改  

```bash
<xacro:arg name="odom" default="false"/> 
改為true
```
